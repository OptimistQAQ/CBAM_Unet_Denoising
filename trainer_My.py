...
from utils import *
from mipi_starting_kit.code_example.dataset import *
from archs import *
from losses import *
    ...
    def __init__(self):
        ...
        if self.mode=='train':
            opt = {
                ...
                'dataroot': 'F:\\denoise_codalab\\PMN\\data\\train\\Camera1\\',         # the path to our released dataset
                'data_pair_list': 'F:\\denoise_codalab\\PMN\\data\\train\\Camera1\\train.txt', # the path to the txt file of the mipi dataset
            ...
            self.dataset = MIPIDataset(opt)
            self.dst_train = globals()[self.args['dst_train']['dataset']](self.args['dst_train'])
            self.dataloader_train = DataLoader(self.dataset, batch_size=self.hyper['batch_size'], worker_init_fn=self.worker_init_fn,
        ...
        # self.loss = Unet_Loss()
        self.loss = torch.nn.SmoothL1Loss(size_average=None, reduce=None, reduction='mean', beta=1.0)
        ...
        for epoch in range(self.hyper['last_epoch']+1, self.hyper['stop_epoch']+1):
            ...
            with tqdm(total=len(self.dataloader_train)) as t:
                for k, data in enumerate(self.dataloader_train):
                    ...
                    imgs_lr = data['lq'].to(self.device)
                    imgs_hr = data['gt'].to(self.device)
                    ratio = data['ratio']
